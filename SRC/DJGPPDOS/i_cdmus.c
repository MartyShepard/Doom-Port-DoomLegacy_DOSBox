...
//-----------------------------------------------------------------------------
//
// $Id: i_cdmus.c 1257 2016-09-20 17:14:21Z wesleyjohnson $
//
...
//
// DESCRIPTION:
//      cd music interface (uses bcd library)
//      
//-----------------------------------------------------------------------------
...
consvar_t   cdUpdate  = {"cd_update","1",CV_SAVE,CV_OnOff};


// hour,minutes,seconds
static char* hms(int hsg)
{
    ...
    if (cdPlaying)
    {
        if (!bcd_audio_busy())
        {
            ...
            else
            {
                ...
                cdPlayTrack++;
                if (cdPlayTrack > TrkOrder.highest_track)
                    cdPlayTrack = TrkOrder.lowest_track;
                while (!tracks[cdPlayTrack].is_audio && cdPlayTrack< TrkOrder.highest_track)
                      cdPlayTrack++;
                I_PlayCD (cdPlayTrack, true);
            }
        }
...
void I_PlayCD (unsigned int track, boolean looping)
{
    ...
    if (track < TrkOrder.lowest_track || track > TrkOrder.highest_track)
    {
        ...
        track = (track % (TrkOrder.highest_track- TrkOrder.lowest_track+1)) + 1;
        //return;
    }
